...
func (s *PrivateAccountAPI) signTransaction(ctx context.Context, args *TransactionArgs, passwd string) (*types.Transaction, error) {
	...
	return wallet.SignTxWithPassphrase(account, passwd, tx, s.b.ChainConfig().GetChainId(s.b.CurrentHeader().Number))
}
...
func (api *PublicBlockChainAPI) ChainId() (*hexutil.Big, error) {
	...
	if config := api.b.ChainConfig(); config.IsEIP155(api.b.CurrentBlock().Number()) {
		return (*hexutil.Big)(config.GetChainId(api.b.CurrentHeader().Number)), nil
	}
...
func (s *PublicTransactionPoolAPI) sign(addr common.Address, tx *types.Transaction) (*types.Transaction, error) {
	...
	return wallet.SignTx(account, tx, s.b.ChainConfig().GetChainId(s.b.CurrentHeader().Number))
}
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args TransactionArgs) (common.Hash, error) {
	...
	signed, err := wallet.SignTx(account, tx, s.b.ChainConfig().GetChainId(s.b.CurrentHeader().Number))
	if err != nil {
		return common.Hash{}, err
	}
	return SubmitTransaction(ctx, s.b, signed)
...
func metaTransactionCheck(ctx context.Context, tx *types.Transaction, b Backend) error {
	if types.IsMetaTransaction(tx.Data()) {
		...
		addr, err := metaData.ParseMetaData(tx.Nonce(), tx.GasPrice(), tx.Gas(), tx.To(), tx.Value(), metaData.Payload, from, b.ChainConfig().GetChainId(b.CurrentHeader().Number))
		if err != nil {
			return err
		}
