...
func (c *Congress) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if header.Difficulty.Cmp(diffNoTurn) == 0 {
		...
		randWiggle := rand.Int63n(int64(wiggle))
		if randWiggle < int64(3*time.Second) {
			randWiggle = int64(3 * time.Second)
		}
		delay += time.Duration(randWiggle)
...
func (c *Congress) PreHandle(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	if c.chainConfig.BlackListBlock != nil && c.chainConfig.BlackListBlock.Cmp(header.Number) == 0 {
		return systemcontract.ApplySystemContractUpgrade(systemcontract.SysContractV0, state, header, newChainContext(chain, c), c.chainConfig)
	}
	...
	if c.chainConfig.EvolveBrandBlock != nil && c.chainConfig.EvolveBrandBlock.Cmp(header.Number) == 0 {
		return systemcontract.ApplySystemContractUpgrade(systemcontract.WAIAContractV1, state, header, newChainContext(chain, c), c.chainConfig)
	}
...
// SetSigner is set signer based on block number
func (c *Congress) SetSigner(blockNumber *big.Int) {
	c.signer = types.LatestSignerForChainID(c.chainConfig.GetChainId(blockNumber))
	log.Info("Engine set signer", "on blockNumber", blockNumber)
}
