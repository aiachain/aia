...
func Transaction(ctx *cli.Context) error {
	...
	for it.Next() {
		...
		if gas, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil,
			chainConfig.IsHomestead(new(big.Int)), chainConfig.IsIstanbul(new(big.Int)), chainConfig.IsShanghaiVM(new(big.Int))); err != nil {
			r.Error = err
			results = append(results, r)
			continue
		} else {
		...
		if chainConfig.IsShanghaiVM(new(big.Int)) && tx.To() == nil && len(tx.Data()) > params.MaxInitCodeSize {
