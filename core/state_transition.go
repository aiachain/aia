...
func (st *StateTransition) metaTransactionCheck() error {
	if types.IsMetaTransaction(st.data) {
		...
		chainID := st.evm.ChainConfig().GetChainId(st.evm.Context.BlockNumber)
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	gas, err := IntrinsicGas(st.data, st.msg.AccessList(), contractCreation, homestead, istanbul, rules.IsShanghaiVM)
	if err != nil {
		return nil, err
	}
	if st.gas < gas {
	...
	if rules.IsShanghaiVM && contractCreation && len(st.data) > params.MaxInitCodeSize {
