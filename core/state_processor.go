...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	if p.config.IsNewChainIDEqual(blockNumber) {
		p.bc.TxPool().ResetSigner(blockNumber)
		if isPoSA {
			posa.SetSigner(blockNumber)
		}
	}
	...
	for i, tx := range block.Transactions() {
		if i == 0 && p.config.IsEvolveBrand(header.Number) {
			...
			if tx.Value().Cmp(types.CoinbaseReward(p.config, header.Number)) != 0 {
				return nil, nil, 0, fmt.Errorf("Incorrect coinbase transaction value")
			}
		} else {
			if isPoSA {
