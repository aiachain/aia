...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool := &TxPool{
		...
		chain:           chain,
		signer:          types.LatestSigner(chainconfig, chain.CurrentBlock().Header().Number),
...
func (pool *TxPool) addTxsLocked(txs []*types.Transaction, local bool) ([]error, *accountSet) {
	if pool.chainconfig.IsNewChainIDEqual(pool.chain.CurrentBlock().Header().Number) {
		pool.ResetSigner(pool.chain.CurrentBlock().Header().Number)
	}
...
func (pool *TxPool) ResetSigner(number *big.Int) {
	pool.signer = types.LatestSigner(pool.chainconfig, pool.chainconfig.GetChainId(number))
	pool.locals.resetSigner(pool.signer)
	log.Info("Chain id change", "old chainid", pool.chainconfig.ChainID, "new chainid", pool.signer.ChainID())
}
...
func (as *accountSet) resetSigner(signer types.Signer) {
	as.signer = signer
}
